...
Yes we modified it to a dense layer with two outputs, a score for each class (covid and normal).
to align with Grad-CAM’s requirement .

Question 20

Do you think that having only two class labels affects the explanation? If you do, how so?

Before the scalar told us which decision was made which is binary (covid or not). But now the interpretation change a bit, we now have a logit or porbability score for both the classes. 
You could say that the highest probability score is the class that is chosen, which will be equivalent to the binary classification in a way.
...
Yes, if we set pred_index to zero (covid class relevant) you can see that the model is focusing on the lung areas, which is medically relevant probably.

Question 22

Did Grad-CAM visualizations help you identify potential biases in the model?

We can observe activation sometimes to a square area in the image, which is not relevant to the classification? 
