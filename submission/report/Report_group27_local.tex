...
In this task, we selected Recall and Precision as the appropriate metrics because accurately identifying COVID cases is critical, and misclassifying COVID cases as "normal" (false negatives) could have serious implications. These metrics help evaluate the model's performance by emphasizing the correct identification of COVID cases, which is crucial for this problem.
Looking at the training and validation curves in the initial model's history plot, it was evident that the model was overfitting. This is shown by the high training accuracy compared to the significantly lower validation accuracy, indicating that the model performs well on the training data but struggles with unseen data. The learning rate emerged as the hyperparameter that would need to be optimized first to strike a balance between training speed and model accuracy.Overfitting was observed due to the significant gap between training and validation accuracy. To address this, we implemented countermeasures like early stopping, where training is halted when the validation loss no longer improves, and dropout layers to randomly deactivate neurons, helping to reduce overfitting. The Adam optimizer was also used, which helps with both convergence and handling overfitting by adjusting the learning rate during training.
For hyperparameter search, we began with a grid search to test a small subset of values for key parameters such as learning rate, batch size, and dropout rate. This helped identify a good range of values. In case of a large hyperparameter space, we also employed random search to sample random combinations, which is often more efficient. After initial testing, we used sequential tuning, adjusting hyperparameters based on previous results, such as fine-tuning the learning rate after optimizing the batch size.
The Adam optimizer was chosen for this task due to its ability to combine the advantages of both Momentum and RMSprop, enabling faster convergence and better handling of the learning rate. For the loss function, binary cross-entropy was selected, as it is suitable for binary classification tasks like this one. While we also experimented with RMSprop and SGD optimizers, Adam was found to be the most effective. For multi-class tasks, categorical cross-entropy could be considered as an alternative loss function.
